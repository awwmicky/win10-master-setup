...
$Tweaks = @(
	...
	### Service Tweaks ###
	"DisableUpdateMsftOffer",       # "EnableUpdateMsftOffer",
...
Function TitusRegistryTweaks {
	Write-Output "Improving Windows Update to delay Feature Updates..."
	Write-Output "Only Install Windows Security (Quality) Updates"
	### Apply Windows Update to delay feature updates and only update at certain times
	$Path = "HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings"
	
    If (!(Get-ItemProperty -Path $Path -Name "BranchReadinessLevel" -ErrorAction SilentlyContinue)) { New-ItemProperty -Path $Path -Name "BranchReadinessLevel" -Type DWord -Value 20 }
	Set-ItemProperty -Path $Path -Name "BranchReadinessLevel" -Type DWord -Value 20

	If (!(Get-ItemProperty -Path $Path -Name "DeferFeatureUpdatesPeriodInDays" -ErrorAction SilentlyContinue)) { New-ItemProperty -Path $Path -Name "DeferFeatureUpdatesPeriodInDays" -Type DWord -Value 30	}
	Set-ItemProperty -Path $Path -Name "DeferFeatureUpdatesPeriodInDays" -Type DWord -Value 30

	If (!(Get-ItemProperty -Path $Path -Name "DeferQualityUpdatesPeriodInDays" -ErrorAction SilentlyContinue)) { New-ItemProperty -Path $Path -Name "DeferQualityUpdatesPeriodInDays" -Type DWord -Value 7 }
	Set-ItemProperty -Path $Path -Name "DeferQualityUpdatesPeriodInDays" -Type DWord -Value 7

	If (!(Get-ItemProperty -Path $Path -Name "ActiveHoursEnd" -ErrorAction SilentlyContinue)) { New-ItemProperty -Path $Path -Name "ActiveHoursEnd" -Type DWord -Value 2 }
	Set-ItemProperty -Path $Path -Name "ActiveHoursEnd" -Type DWord -Value 2

	If (!(Get-ItemProperty -Path $Path -Name "DeferQualityUpdatesPeriodInDays" -ErrorAction SilentlyContinue)) { New-ItemProperty -Path $Path -Name "ActiveHoursStart" -Type DWord -Value 8 }
	Set-ItemProperty -Path $Path -Name "ActiveHoursStart" -Type DWord -Value 8
}
...
Function InstallTitusProgs {
    ...
    Write-Output "Running O&O Shutup (with Custom Settings)..."
	Import-Module BitsTransfer
    Start-BitsTransfer -Source $env:USERPROFILE\Desktop\win10script-master\ooshutup10.cfg -Destination ooshutup10.cfg
...
Function InstallAdobe {
	Show-Choco-Menu -Title "Adobe Acrobat Reader" -ChocoInstall "adobereader"
}

Function Install7Zip {
	Show-Choco-Menu -Title "7-Zip" -ChocoInstall "7zip"
}

Function InstallNotepadplusplus {
	Show-Choco-Menu -Title "Notepad++" -ChocoInstall "notepadplusplus"
}

Function InstallVLC {
	Show-Choco-Menu -Title "VLC" -ChocoInstall "vlc"
}

Function InstallIrfanview {
	Show-Choco-Menu -Title "Irfanview" -ChocoInstall "irfanview"
}

Function InstallVSCode {
	Show-Choco-Menu -Title "Visual Studio Code" -ChocoInstall "vscode"
}

Function InstallWinTerminal {
	Show-Choco-Menu -Title "Microsoft Windows Terminal" -ChocoInstall "microsoft-windows-terminal"
}

Function InstallPowerToys {
	Show-Choco-Menu -Title "PowerToys" -ChocoInstall "powertoys"
}
...
# i.e. if apps can download or update when they aren't used
# Cortana is excluded as its inclusion breaks start menu search
...
# Needed only for 1507 as local P2P is the default since 1511
...
# Lower UAC level - disabling secure confirm, it completely would break apps
Function SetUACLow {
...
# Access Network File Sharing Protocol
Function DisableSMBV1 {
	Write-Output "Disabling SMB 1.0/CIFS Client Protocol..."
...
Function EnableSMBV1 {
	Write-Output "Enabling SMB 1.0 Protocol..."
	Enable-WindowsOptionalFeature -Online -FeatureName "SMB1Protocol-Client" -NoRestart -WarningAction SilentlyContinue | Out-Null
...
# Disable SMB Server
# Completely disables file and printer sharing, but leaves the system able to connect to another SMB server as a client
...
# Applicable to 1709 or newer, requires Windows Defender to be enabled
...
# Part of Windows Defender System Guard virtualization-based security - Supported from 1803
...
Function DisableUpdateMsftOffer {
...
Function EnableUpdateMsftOffer {
...
Function DisableHomeGroups {
	...
	If ([System.Environment]::OSVersion.Version.Build -lt 17135) {    
...
# Disable Sleep in Start Menu and keyboard button
Function DisableSleepButton {
	Write-Output "Disabling Sleep in Start Menu and keyboard button..."
	If (!(Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FlyoutMenuSettings")) {
		New-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FlyoutMenuSettings" | Out-Null
	}
...
Function UninstallWindowsStore {
    ...
    # EDIT: "Microsoft.StorePurchaseApp"
...
Function UninstallLegacyComponents {
	Write-Output "Uninstalling Legacy Components..."
...
Function InstallLegacyComponents {
	Write-Output "Installing Legacy Components..."
	Enable-WindowsOptionalFeature -Online -FeatureName "LegacyComponents" -NoRestart -WarningAction SilentlyContinue | Out-Null
}
...
# Access Sync Share from Work File Servers 
...
Function EnablePasswordPolicy {
	...
	secedit /configure /db "$env:SYSTEMROOT\security\database\local.sdb" /f $tmpfile /areas SECURITYPOLICY | Out-Null
	Remove-Item -Path $tmpfile
}
...
Function WaitForKey {
	...
	[Console]::ReadKey($true) | Out-Null # BROKEN
}
...
# Run is completed
Function Complete {
    Write-Output "< Minimal script has been completed. >"
}
...
Function RemoveOpenFileToMsftEdge {
    ...
    $PackagesPath = "HKCU:\SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\CurrentVersion\AppModel\Repository\Packages"
    $MsftEdgePath = Get-ChildItem $PackagesPath -Recurse -include "MicrosoftEdge"
    #Write-Output $MSEdgePath

    #Specify the paths to the file and URL associations 
    $FileAssocKey = Join-Path $MsftEdgePath.PSPath "Capabilities\FileAssociations"
    #Write-Output $FileAssocKey
    ...
    #Write-Output $FileAssoc

    $Associations = @() 
    $Filetypes.Property | ForEach { $Associations += $FileAssoc.$_ } 
    #Write-Output $Associations

    #Add registry values in each software class to stop Edge as the default 
    ForEach ($Association in $Associations) 
    { 
        $Class = Join-Path "HKCU:SOFTWARE\Classes" $Association 
        #If (Test-Path $class) { Write-Host $Association } 
...
# Remove "Edit/Print with Paint 3D" from Context Menu
Function RemovePaint3DContextMenu {
    Write-Output "Removing Paint 3D from Context Menu..."
    Remove-Item -Path "HKCR:\SystemFileAssociations\.3mf\Shell\3D Print" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.3mf\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.bmp\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.fbx\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.gif\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.jfif\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.jpe\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.jpeg\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    ...
    Remove-Item -Path "HKCR:\SystemFileAssociations\.tif\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path "HKCR:\SystemFileAssociations\.tiff\Shell\3D Edit" -Recurse -ErrorAction SilentlyContinue
}
